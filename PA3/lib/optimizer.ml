...
let undo_singe_static_assignment (method_graph : Cfg.cfg) =
  let unssaify (tac : Tac.tac_elem) : Tac.tac_elem =
    let revert_string (str : string) =
      let split = String.split_on_char '$' str in
      if List.length split > 1 then "t$" ^ List.nth split 1 else str
    in
...
let singe_static_assignment (method_graph : Cfg.cfg) =
  ...
  let rec ssaify_cfg_node (elem : cfg_elem) : cfg_elem =
    ...
    | If_Statement (cond_stmt, then_stmt, else_stmt, join_stmt, phi) ->
        ...
        List.iter
          (fun (k, v) ->
            Printf.fprintf Print.debug_file "\tChanged: %s \n" k;
            Tac.print_tac_elems_file v Print.debug_file)
          changed_values;*)
        (* let else_hash = get_hash_values () in *)
        (* PHI:  Get diff of ssa_then and ssa_else, create list of changed values *)
        (* phi_function then_hash else_hash cond_hash; *)
